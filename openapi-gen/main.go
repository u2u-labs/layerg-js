...
export interface {{$classname | title}} {
  ...
  {{$fieldname}}?: Record<string, number>;
...
{{- range $url, $path := .Paths}}
  {{- range $method, $operation := $path}}
    {{- $opId := $operation.OperationId | stripOperationPrefix | snakeToCamel }}

  /** {{$operation.Summary}} */
  {{- if eq $opId "rpcFunc" }}
  {{ $opId }}(bearerToken: string,
      id:string,
      body:string,
      httpKey?:string,
...
func enumDescriptions(def Definition) (output []string) {
	...
	// If we have a description with newlines, use it
	if len(split) > 1 {
		return split[1:]
	}

	// Otherwise return empty descriptions
	return make([]string, len(def.Enum))
}
