...
/* A validated subscription stored by LayerG. */
...
export class Client {
  ...
  /** The low level API client for LayerG server. */
  private readonly apiClient: LayerGApi;
  ...
  authenticateEmail(email: string, otp: string, create?: boolean, vars?: Record<string,string>): Promise<Session> {
    const account: ApiAccountEmail = {
      email: email,
      vars: vars
    };

    return this.apiClient.authenticateEmail("", "", account, create, otp).then((apiSession : ApiSession) => {
      return new Session(apiSession.token || "", apiSession.refresh_token || "", apiSession.created || false);
    });
  }
  ...
  authenticateUA(code: string, state: string, source: number, vars?: Record<string,string>): Promise<Session> {
    ...
    return this.apiClient.authenticateUA(this.serverkey, "", request).then((apiSession : ApiSession) => {
      return new Session(apiSession.token || "", apiSession.refresh_token || "", apiSession.created || false);
    });
  }
 ...
 async banGroupUsers(session: Session, groupId: string, ids?: Array<string>): Promise<boolean> {
  ...
  async sendEmailOTP(email: string): Promise<boolean> {
    const request: ApiSendEmailOTPRequest = {
      email: email
    };
  ...
  async sendTelegramOTP(telegramId: string): Promise<boolean> {
    const request: ApiSendTelegramOTPRequest = {
